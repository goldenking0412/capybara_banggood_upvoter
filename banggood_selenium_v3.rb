...
class Amazon
  def initialize
    ...
    @question_users = [
      {email: 'satahari@getdll.com', password: '147963'},
      {email: 'samuel@getdll.com', password: '147963'},
  ...
  def login(user)
    ...
    begin
      ...
      puts 'Something went wrong on banggood. Please check!'
    end
  end
  ...
  def logout
    ...
    sleep 3
    @session.visit "https://www.banggood.com/index.php?com=account&t=logout" if current_is_login?
    puts 'Logged out'
  rescue
    @session.visit "https://www.banggood.com/index.php?com=account&t=logout"
  end
  ...
  def get_product_number(product_url)
    separates = product_url.delete_suffix('.html').split('-')
    return separates[separates.length - 1]
  end
  ...
  def post_question(url, user)
    ...
    if @session.has_text?('xxx')
      ...
      product_number = get_product_number(url)
      @session.visit 'https://www.banggood.com/qa-question-'+product_number+'-1.html'
      puts 'product question url' + 'https://www.banggood.com/qa-question-'+product_number+'-1.html'
      sleep 3

      begin
        @session.find('.qa_lead_new')
      rescue Exception => e
  ...
  def vote_question(question)
    ...
    @session.all('.answers-bd-list') do |element|
      ...
      if answer.include? question[:answer]
